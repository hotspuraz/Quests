...
public class StorylineInventory implements InventoryProvider {
   public static void open(Player player, int page) {
      SmartInventory.builder()
         .title(ChatColor.translateAlternateColorCodes('&', Objects.requireNonNull(Quests.getInstance().getConfig().getString("items.storyline.title"))))
         .size(3, 9)
   ...
   public void init(Player player, InventoryContents contents) {
      ...
      if (user != null) {
         ...
         Quest first = Quests.getInstance().getQuestController().firstStory();
         if (first != null) {
            quests.add(first);
