...
public class DailyInventory implements InventoryProvider {
   public static void open(Player player) {
      SmartInventory.builder()
         .title(ChatColor.translateAlternateColorCodes('&', Objects.requireNonNull(Quests.getInstance().getConfig().getString("items.daily.title"))))
         .size(3, 9)
   ...
   public void init(Player player, InventoryContents contents) {
      ...
      if (user != null) {
         List<Quest> quests = Quests.getInstance().getQuestController().getDailyQuests();
