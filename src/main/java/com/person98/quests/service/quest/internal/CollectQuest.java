...
public class CollectQuest extends QuestContainer {
   ...
   public void death(PlayerDeathEvent event) {
      ...
      for (Entity entity : player.getNearbyEntities(3.0, 3.0, 3.0)) {
         if (entity instanceof Item item) {
            item.setMetadata("collectQuest", new FixedMetadataValue(Quests.getInstance(), 10));
         }
      }
   }
   ...
   public void drop2(BlockDropItemEvent event) {
      if (event.getBlock().hasMetadata("collectQuest")) {
         event.getItems().forEach(item -> item.setMetadata("collectQuest", new FixedMetadataValue(Quests.getInstance(), 10)));
      }
   }

   @EventHandler
   public void drop(PlayerDropItemEvent event) {
      event.getItemDrop().setMetadata("collectQuest", new FixedMetadataValue(Quests.getInstance(), 10));
   }
}
