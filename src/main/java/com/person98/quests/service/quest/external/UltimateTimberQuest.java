package com.person98.quests.service.quest.external;
...
public class UltimateTimberQuest extends QuestContainer {
   ...
   public static void register(Quests plugin) {
   ...
   private void onTreeFall(Event event) {
      try {
         ...
         Quests.getInstance().getLogger().warning("Could not process UltimateTimber tree fall: " + exception.getMessage());
      }
   }
