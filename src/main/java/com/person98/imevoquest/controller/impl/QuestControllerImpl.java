...
import com.person98.imevoquest.data.stage.RootType;
import com.person98.imevoquest.data.stage.Stage;
import com.person98.imevoquest.data.stage.impl.StageData;
import com.person98.imevoquest.util.ConfigurationHelper;
import java.io.File;
...
public class QuestControllerImpl implements QuestController {
   ...
   public void loadPersistedDailyQuests() {
      this.dailyQuestFile = new File(ImevoQuest.getInstance().getDataFolder(), "daily_quests.yml");
   ...
   public void ensureDailyQuestsUpToDate() {
      String resetTime = ImevoQuest.getInstance().getConfig().getString("reset-time", "00:00");
      ...
      if (shouldReset) {
         ...
         if (ImevoQuest.getInstance().getUserController() != null) {
            ImevoQuest.getInstance().getUserController().resetAllDailyQuestProgress();
         }
      ...
      if (files != null) {
         for (File file : files) {
            if (file.isDirectory()) {
               ...
               String parent = "plugins" + File.separator + "ImevoQuests" + File.separator;
               ConfigurationHelper configuration = new ConfigurationHelper(ImevoQuest.getInstance(), file.getParent().replace(parent, ""), file.getName());
